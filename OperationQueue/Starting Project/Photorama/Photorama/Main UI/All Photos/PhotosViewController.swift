...
class PhotosViewController: UIViewController, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        store.fetchImage(for: photo, completion: { (result) -> Void in
            ...
            let faceFuzzAction = ImageProcessor.Action.pixelateFaces
            ...
            let request =
                self.imageProcessor.process(image: image, priority: .high, actions: actions) { (thumbResult) in
                        ...
                        case let .failure(error):
                            ...
                            switch error {
                                case ImageProcessor.Error.cancelled:
                                    print("Cancelled processing photo \(photoID)")
                                default:
                                    print("Failed tp process photo \(photoID): \(error)")
                            }
                            thumbnail = image
